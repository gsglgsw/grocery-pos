        ...
        try {
            ...
            class PosController {
                ...
                async init() {
                    ...
                    this.view.bindCustomerCRMActions(this.handleEditCustomer.bind(this), this.handleDeleteCustomer.bind(this));
               ...
               // 🚀 Phase 8.2 升級：處理全域資料同步，精準控制 Event Loop 確保遮罩渲染
    async handleSyncProducts() {
        try {
            this.view.showLoader(); // 🔒 鎖定螢幕，觸發 DOM 變更

            // 🚀 Tech Lead 注入 1：強制最少等待 500 毫秒 (Promise.all)。
            // 即使筆電網路瞬間完成，也能保證遮罩至少顯示 0.5 秒，避免視覺閃爍 (Flicker)
            const [results] = await Promise.all([
                this.model.syncAllDataFromCloud(),
            ...
            this.view.hideLoader(); // 🔓 解除鎖定，觸發 DOM 隱藏
            
            // 🚀 Tech Lead 注入 2：強制讓出主執行緒 (setTimeout 100ms)
            // 將 alert 推入下一個 Event Loop，確保瀏覽器有足夠時間把「隱藏遮罩」的 CSS 畫到螢幕上
            setTimeout(() => {
                alert(`✅ 災難復原同步完成！\n\n已成功從雲端下載並修復：\n📦 商品資料：${results.products} 筆\n👥 顧客檔案：${results.customers} 筆\n📖 帳本明細：${results.ledgers} 筆\n🧾 近期訂單：${results.orders} 筆`);
            }, 100);
            ...
            setTimeout(() => {
                ...
                async handleAddCustomer() {
                    ...
                    } catch (error) { alert('新增失敗，請聯絡系統管理員。'); }
                }
                ...
                handleCashCheckout() {
                    ...
                    this.posChannel.postMessage({ action: 'SHOW_CHANGE', data: { change: result.change } });
                }
                ...
                async handleCreditCheckout(customer) {
                    ...
                    } catch (error) { alert('掛帳失敗：資料庫寫入異常。'); }
                }
                ...
                async handleCloseModal() {
                    try {
                        ...
                        this.updateView();
                    } catch (error) { alert('系統錯誤：資料寫入失敗，請勿關閉網頁並通知管理員。'); }
                }

                initScannerListener() {
                    const SCANNER_TIMEOUT_MS = 1000;
                ...
                async processBarcode(barcode) { await this.processProductEntry(barcode); }
                ...
                async processProductEntry(barcode) {
                    ...
                    if (!result.success && result.reason === 'unknown_barcode') {
                        ...
                        if (isNaN(customPrice) || customPrice < 0) { alert('❌ 金額輸入錯誤，請重新操作。'); return; }
                ...
                async handleQuickKey(barcode) {
                    ...
                    if (!product) { alert(`❌ 系統錯誤：找不到快捷商品檔案 (條碼: ${barcode})！`); return; }

                    let customPrice = null;
                    if (product && product.isOpenPrice) {
                        customPrice = this.model.getNumpadValue();
                        if (customPrice === null) { this.view.showNumpadError(); return; }
                    }
                    const result = await this.model.addToCart(barcode, customPrice);
                    if (result.success) { this.model.clearNumpad(); this.updateView(); }
                }
                ...
                updateView() {
                    ...
                    this.posChannel.postMessage({ action: 'UPDATE_CART', data: cartReport });
                }
                ...
                async handleSearchProduct(barcode) {
                    ...
                    } else { alert(`ℹ️ 資料庫無此商品，請建立新檔案。`); }
                }

                async handleSaveProduct(productData, closeCallback) {
                    if (!productData.barcode || !productData.name) { alert('❌ 儲存失敗：請務必填寫「商品條碼」與「商品名稱」'); return; }
                    const result = await this.model.saveProduct(productData);
                    if (result.success) {
                        alert(`✅ 成功建檔：${productData.name}\n(系統已自動將其加入搜尋預覽資料庫)`);
                        closeCallback();
                        this.updateView();
                    } else { alert(`❌ 儲存發生錯誤：${result.message}`); }
                }
                ...
                async handleDeleteCustomer(customerId, name, debt) {
                    if (debt > 0) { alert(`⚠️ 無法刪除：顧客「${name}」目前尚有 $${debt} 欠款未結清！\n請先辦理還款結清後再進行刪除。`); return; }
                ...
                startCameraScanner() {
                    if (this.html5QrcodeScanner) this.stopCameraScanner();
                    ...
                    ).catch((err) => { alert("無法啟動相機，請檢查瀏覽器權限設定。"); });
                }
