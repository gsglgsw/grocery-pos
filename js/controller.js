...
class PosController {
    ...
    // 🚀 Phase 8.2 升級：處理全域資料同步，加入 Loader 防止誤觸，並進行嚴格視圖重整
    async handleSyncProducts() {
        try {
            this.view.showLoader(); // 🔒 鎖定螢幕

            const results = await this.model.syncAllDataFromCloud();
            
            // 🔄 重整全域視圖，確保所有畫面與最新資料庫一致
            this.updateView(); 
            const ledgers = await this.model.getLedgerSummary();
            this.view.renderLedgerList(ledgers); 
            await this.loadAuditToday(); 
            
            this.view.hideLoader(); // 🔓 解除鎖定
            alert(`✅ 災難復原同步完成！\n\n已成功從雲端下載並修復：\n📦 商品資料：${results.products} 筆\n👥 顧客檔案：${results.customers} 筆\n📖 帳本明細：${results.ledgers} 筆\n🧾 近期訂單：${results.orders} 筆`);
            
        } catch (error) {
            this.view.hideLoader(); // 🔓 發生錯誤時也必須解除鎖定
